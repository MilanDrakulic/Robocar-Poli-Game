...
namespace Anima2D.Pool
{
	...
	public abstract class OnionObjectPool< T >
	{
		...
		protected OnionObjectPool()
		{
			m_AvaliableObject = new List<T>();
			m_DispatchedObjects = new List<T>();
		}

		public OnionObjectPool(ICreationPolicy<T> _creationPolicy) : this()
		{
	...
	public class DefaultObjectPool<T> : OnionObjectPool<T> where T : new ()
	{
